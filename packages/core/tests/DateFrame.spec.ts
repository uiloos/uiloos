...
import { DateFrameDate } from '../src/DateFrame/DateFrameDate';
import { DateFrameEvent } from '../src/DateFrame/DateFrameEvent';
...
type DateFrameSansDatesAndEvents<T> = Pick<
  DateFrame<T>,
  'history' | 'firstDayOfWeek' | 'dateFormat' | 'mode'
>;

type TestState<T> = DateFrameSansDatesAndEvents<T> & {
  dates: TestDate<T>[];
  events: TestEvent<T>[];
  activeEvents: TestEvent<T>[];
};

type TestEvent<T> = Pick<DateFrameEvent<T>, 'index'> & {
  data: string;
};

type TestDate<T> = Pick<DateFrameDate<T>, 'index' | 'date' | 'isPadding'> & {
  events: TestEvent<string>[];
};
...
function assertState(state: DateFrame<string>, expected: TestState<string>) {
  const callAsTestState: TestState<string> = {
    ...
    dates: state.dates.map((date) => {
      const dateAsTestDate: TestDate<string> = {
        index: date.index,
        date: date.date,
      ...
      return dateAsTestDate;
    }),
    events: state.events.map(eventToTestEvent),
    activeEvents: state.activeEvents.map(eventToTestEvent),
  };

  expect(callAsTestState).toEqual(expected);
}

function eventToTestEvent(event: DateFrameEvent<string>): TestEvent<string> {
  const eventAsTestEvent: TestEvent<string> = {
    index: event.index,
    data: event.data,
  };

  return eventAsTestEvent;
}
