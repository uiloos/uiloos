...
export class ViewChannel<T, R = void> {
  /**
   * The `ViewChannelView`'s which the `ViewChannel` holds.
   */
  ...
  private _observer: _Observer<ViewChannel<T, R>, ViewChannelEvent<T, R>> = new _Observer();
   ...
   */
  public present(viewConfig: ViewChannelViewConfig<T, R>): ViewChannelView<T, R> {
